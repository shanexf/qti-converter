"""
Builds a QTI content package (zip of assessmentItem XML files + imsmanifest.xml)
from a list of structured question dicts (see parser.py for the shape of each type).

Supports QTI 2.1 and 2.2 — pass version="2.1" or "2.2" to build_qti_package.
Use 2.1 for platforms that haven't caught up yet (e.g. itslearning); 2.2 for
everything else (Canvas, Moodle, Blackboard all accept both).
"""
import io
import zipfile
...
VERSIONS = {
    "2.1": {
        ...
        "xsi": "http://www.imsglobal.org/xsd/imsqti_v2p1 http://www.imsglobal.org/xsd/qti/qtiv2p1/imsqti_v2p1.xsd",
        "resource_type": "imsqti_item_xmlv2p1",
    },
    "2.2": {
        "ns": "http://www.imsglobal.org/xsd/imsqti_v2p2",
        "xsi": "http://www.imsglobal.org/xsd/imsqti_v2p2 http://www.imsglobal.org/xsd/qti/qtiv2p2/imsqti_v2p2.xsd",
        "resource_type": "imsqti_item_xmlv2p2",
    },
}
...
def _item_header(identifier, title, version_info):
    return (
        f'<assessmentItem xmlns="{version_info["ns"]}" '
        f'xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" '
        f'xsi:schemaLocation="{version_info["xsi"]}" '
        f'identifier="{identifier}" title="{_esc(title)}" '
        f'adaptive="false" timeDependent="false">'
    )


def _mc_item(q, identifier, version_info):
    ...
    return (
        _item_header(identifier, q["prompt"][:80], version_info)
...
def _matching_item(q, identifier, version_info):
...
def _fill_blank_item(q, identifier, version_info):
    return (
        _item_header(identifier, q["prompt"][:80], version_info)
...
def _essay_item(q, identifier, version_info):
    return (
        _item_header(identifier, q["prompt"][:80], version_info)
        + f'<outcomeDeclaration identifier="SCORE" cardinality="single" baseType="float">'
        f"<defaultValue><value>0</value></defaultValue></outcomeDeclaration>"
...
def _manifest_xml(items, version_info):
    resources = "".join(
        f'<resource identifier="{iid}" type="{version_info["resource_type"]}" href="{iid}.xml">'
...
def build_qti_package(questions, version="2.2") -> bytes:
    """Returns the raw bytes of a zip file containing the QTI package.

    version: "2.1" or "2.2" (defaults to 2.2). Falls back to 2.2 for any
    unrecognized value rather than failing the whole export.
    """
    version_info = VERSIONS.get(version, VERSIONS["2.2"])
    buf = io.BytesIO()
    items = []
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        for i, q in enumerate(questions):
            ...
            xml = '<?xml version="1.0" encoding="UTF-8"?>' + builder(q, item_id, version_info)
            zf.writestr(f"{item_id}.xml", xml)
            items.append((item_id, q))

        zf.writestr("imsmanifest.xml", _manifest_xml(items, version_info))
